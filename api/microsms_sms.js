...
class Main extends Handler {
  ...
  checkCode () {
    this.axios.get(`https://microsms.pl/api/check.php?userid=${this.payments.microsms_sms_id}&number=${this.number}&code=${this.code}&serviceid=${this.payments.microsms_sms_id}`).then(({data}) => {
      if (data.split(',')[0] === '1') {
        this.checkRcon()
      } else {
